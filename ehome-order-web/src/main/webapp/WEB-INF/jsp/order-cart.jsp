...
<head>
	...
	<script src="/js/region_select.js" type="text/javascript"></script>

	<script>
		$(function () {
			$("#location_a").change(function(){
				var address = $("#location_p").val()+$("#location_c").val()+$("#location_a").val();
				$("input[name = 'orderShipping.receiverState']").val($("#location_p").val());
				$("input[name = 'orderShipping.receiverCity']").val($("#location_c").val());
				$("input[name = 'orderShipping.receiverDistrict']").val($("#location_a").val());
			});

			$("#receiverAddress").change(function(){
				var address = $("#location_p").val()+$("#location_c").val()+$("#location_a").val()+":"+$(this).val();
				$("input[name = 'orderShipping.receiverAddress']").val($(this).val());
			});

			$("#receiverName").change(function(){
				$("input[name = 'orderShipping.receiverName']").val($(this).val());
			});
			$("#receiverMobile").change(function(){
				// alert($("#receiverMobile").val());
				// $("input[name = 'orderShipping.receiverPhone']").val($(this).val());
				$("input[name = 'orderShipping.receiverMobile']").val($(this).val());
			});
		});
	</script>
</head>
...
<div class="orderMain">
	...
	<div id="userAddrId" class="orderTbody ">
		<h3 class="orderTitle">收货人信息：<span class="addrAlter" style="display:inline;"></span></h3>
		<br>
		收件人姓名：<input style="border:1px solid" id="receiverName"/>
		<br><br>
		收件人联系电话：<input style="border:1px solid" id="receiverMobile"/>
		<br>
		<div style="margin-top:10px; margin-bottom:10px">
			地址：
				<select name="location_p" id="location_p">
				</select>
				<select style="margin-left: 10px" name="location_c" id="location_c">
				</select>
				<select style="margin-left: 10px" name="location_a" id="location_a">
				</select>
			...
			&nbsp;&nbsp;详细地址：<input style="border:1px solid" id="receiverAddress"/>
		</div>
		<br>
	</div>
				...
				<div class="shipment">
					<div class="shipOrder" id="ylsorder2">
						<div class="sOrders">
							...
							<div class="sOrders">
								<div class="scon">
									<strong>宜家速运</strong>负责配送。<br>商品下单后尽快为您发货
								</div>
							</div>
