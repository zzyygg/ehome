package cn.ehome.cart.interceptor;
...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //home.取到token，需要调用sso系统的服务，根据token取用户信息
        EhomeResult ehomeResult = tokenService.getUserByToken(token);
        //4.没有取到用户信息。登录过期，直接放行。
        if (ehomeResult.getStatus() != 200) {
            return true;
        }
        //5.取到用户信息。登录状态。
        TbUser user = (TbUser) ehomeResult.getData();
